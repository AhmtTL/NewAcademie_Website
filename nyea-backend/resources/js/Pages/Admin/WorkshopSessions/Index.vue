...
const { filters, isLoading } = useServerSearch('admin.workshop-sessions.index', props.filters);
...
<template>
    ...
    <Head title="Workshop Sessions Management" />
                ...
                <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                    <div class="p-6 bg-white border-b border-gray-200">
                        <div class="flex items-center justify-between mb-6">
                            <div>
                                <h2 class="text-2xl font-bold text-gray-900">Workshop Sessions</h2>
                                <p class="text-gray-600 mt-1">Manage workshop sessions across multiple locations</p>
                            </div>
                            <Link :href="route('admin.workshop-sessions.create')"
                            ...
                            <table class="min-w-full divide-y divide-gray-200">
                                ...
                                <tbody class="bg-white divide-y divide-gray-200">
                                    <tr v-for="session in sessions.data" :key="session.id" class="hover:bg-gray-50">
                                        <Link :href="route('admin.workshop-sessions.show', session.id)">
                                        <td class="px-6 py-4">
                                            <div class="text-sm font-medium text-gray-900">{{ session.program?.title }}
                                            </div>
                                            <div class="text-sm text-gray-500">{{ session.program?.slug }}</div>
                                            <div class="text-xs text-gray-600 mt-1">
                                                📅 {{ formatDateRange(session) }} • {{ session.time }}
                                            </div>
                                        </td>
                                        </Link>
                                        ...
                                        <td class="px-6 py-4">
                                            <div class="text-sm font-medium text-gray-900">{{ session.location }}</div>
                                        ...
                                        <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                            <div class="flex items-center justify-end space-x-2">
                                                ...
                                                <Link :href="route('admin.workshop-sessions.edit', session.id)"
                                                    class="text-gray-600 hover:text-gray-900 bg-gray-100 hover:bg-gray-200 px-3 py-1 rounded transition-colors text-sm">
                                                Edit
                                                </Link>
